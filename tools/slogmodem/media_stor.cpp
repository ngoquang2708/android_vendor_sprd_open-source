...
MediaStorage::MediaStorage(const LogString& top_dir)
	:m_inited {false},
	 m_limit {0},
...
CpDirectory* MediaStorage::create_cp_dir(const LogString& name)
{
	...
	return cp_set->create_cp_dir(name);
}

LogFile* MediaStorage::create_cp_file(const LogString& name)
{
	LogFile* log_file = 0;
	CpDirectory* cp_dir = create_cp_dir(name);
	if (cp_dir) {
