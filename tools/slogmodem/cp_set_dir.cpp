...
CpSetDirectory::~CpSetDirectory()
{
	clear_ptr_container(m_cp_dirs);
}
...
int CpSetDirectory::stat()
{
	...
	while (true) {
		...
		if (!::stat(ls2cstring(d_path), &file_stat) &&
		    S_ISDIR(file_stat.st_mode)) {
			CpDirectory* cp_dir = new CpDirectory(this,
							      LogString(dent->d_name));
			if (!cp_dir->stat()) {
...
{
	int ret = mkdir(ls2cstring(m_path),
...
CpDirectory* CpSetDirectory::create_cp_dir(const LogString& cp_name)
{
				     ...
				     LogFile::LogType t)
{
	...
	CpDirectory* cp_dir = create_cp_dir(cp_name);
	LogFile* f = 0;

	if (cp_dir) {
		// Create the CP directory
