...
CpStateHandler::CpStateHandler(LogController* ctrl,
...
int CpStateHandler::process_data()
{
	...
	switch (evt) {
	case CE_ASSERT:
		controller()->process_cp_assert(type);
		break;
	case CE_BLOCKED:
		controller()->process_cp_blocked(type);
		break;
	case CE_ALIVE:
		controller()->process_cp_alive(type);
		break;
	default:
		info_log("unknown event %d", static_cast<int>(evt));
		break;
	}
