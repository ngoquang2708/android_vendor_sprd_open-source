...
public:
	...
	const LogString& base_name() const
	{
		return m_base_name;
	}
	 ...
	 */
	bool operator < (const LogFile& f) const
	{
		return m_time < f.m_time;
	}
	...
	static int parse_log_file_num(const char* name, size_t len,
...
private:
	 ...
	 */
	static int parse_time_string(const char* name, size_t len,
				     FileTime& ft);
	 ...
	 *  Return 0 on success, -1 on error.
	 */
	static int parse_dump_file_time(const char* name, size_t len,
					FileTime& ft);

	static const char* get_4digits(const char* s, size_t len);
	static int extract_file_time(const char* name, size_t len,
				     FileTime& ft);
};
