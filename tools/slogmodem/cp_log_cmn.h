...
#define LOG_TAG "SLOGCP"
	...
	inline const char* ls2cstring(const LogString& str)
	{
		return str.c_str();
	}

	inline void str_assign(LogString& str, const char* s,
			       size_t n)
	{
		str.assign(s, n);
	}

	inline bool str_empty(const LogString& s)
	{
		return s.empty();
	}
...
struct modem_timestamp
{
	...
	unsigned long sys_cnt;            /* modem's time */
};
