...
DeviceFileHandler::DeviceFileHandler(const LogString& dev_path,
				     size_t buf_len,
				     LogController* ctrl,
...
DeviceFileHandler::DeviceFileHandler(int fd,
				     size_t buf_len,
				     LogController* ctrl,
				     Multiplexer* multiplexer)
	:FdHandler(fd, ctrl, multiplexer),
	 m_close_fd {false}
{
	m_buffer.buffer = new uint8_t[buf_len];
	m_buffer.buf_size = buf_len;
...
{
	if (!(events & POLLIN)) {
		err_log("file %s unexpected event %#x",
			ls2cstring(m_file_path), events);
		return;
	}
	...
	while (true) {
		...
		ssize_t n = ::read(m_fd, m_buffer.buffer + free_start,
				   free_size);
		if (-1 == n) {
			...
			if (EAGAIN != errno && ENODATA != errno) {
				err_log("read %s error",
					ls2cstring(m_file_path));
			}
			break;
		}
		if (!n) {
			err_log("read %s return 0",
				ls2cstring(m_file_path));
			break;
		}
		m_buffer.data_len += n;
