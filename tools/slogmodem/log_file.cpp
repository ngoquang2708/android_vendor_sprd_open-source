...
LogFile::LogType LogFile::get_type()
{
	...
	if (!parse_number(reinterpret_cast<const uint8_t*>(s),
			  m_base_name.length(), num, len)) {
		// The name start with a number
		if (len < m_base_name.length() && '-' == s[len]) {
			++len;
			if (!extract_file_time(s + len,
					       m_base_name.length() - len,
					       m_time)) {
				t = LT_LOG;
			}
		}
		...
		if (!parse_time_string(s + 10, m_base_name.length() - 10,
				       m_time)) {
			t = LT_MINI_DUMP;
		}
	} else if (str_starts_with(m_base_name, "sblock_", 7)) {
		if (!parse_time_string(s + 7, m_base_name.length() - 7,
				       m_time)) {
			t = LT_SIPC;
		}
	} else if (str_starts_with(m_base_name, "sbuf_", 5)) {
		if (!parse_time_string(s + 5, m_base_name.length() - 5,
				       m_time)) {
			t = LT_SIPC;
		}
			       ...
			       LogFile::FileTime& ft)
{
	...
	if (parse_number(reinterpret_cast<const uint8_t*>(name + 5),
			 2, num)) {
		return -1;
	}
	ft_tmp.month = static_cast<int>(num);

	if (parse_number(reinterpret_cast<const uint8_t*>(name + 8),
			 2, num)) {
		return -1;
	}
	ft_tmp.mday = static_cast<int>(num);

	if (parse_number(reinterpret_cast<const uint8_t*>(name + 11),
			 2, num)) {
		return -1;
	}
	ft_tmp.hour = static_cast<int>(num);

	if (parse_number(reinterpret_cast<const uint8_t*>(name + 14),
			 2, num)) {
		return -1;
	}
	ft_tmp.min = static_cast<int>(num);

	if (parse_number(reinterpret_cast<const uint8_t*>(name + 17),
			 2, num)) {
		return -1;
	}
				  ...
				  LogFile::FileTime& ft)
{
	...
	while (name < endp) {
		...
		len = endp - p;
		if (len >= 4 &&
		    !parse_number(reinterpret_cast<const uint8_t*>(p),
				  4, num)) {
			is_year = true;
			break;
		}
...
int LogFile::rotate(const LogString& par_path)
{
	...
	int ret = parse_log_file_num(ls2cstring(m_base_name),
				     m_base_name.length(),
				     num, len);
	if (ret) {
		err_log("invalid log file name %s",
			ls2cstring(m_base_name));
		return -1;
	}
...
int LogFile::parse_log_file_num(const char* name, size_t len,
				unsigned& num, size_t& nlen)
{
	int ret = parse_number(reinterpret_cast<const uint8_t*>(name),
			       len, num, nlen);
	if (ret) {
		return -1;
	}
...
int LogFile::change_name_num(unsigned num)
{
	...
	if (parse_log_file_num(ls2cstring(m_base_name), m_base_name.length(),
			       cur_num, len)) {
		return -1;
	}
	...
	char s[32];
	int num_len = snprintf(s, 32, "%u", num);
	LogString new_name;
	
	str_assign(new_name, s, num_len);
	new_name += (ls2cstring(m_base_name) + len);

	LogString par_path = m_dir->cp_set_dir()->path() + "/"
			     + m_dir->name() + "/";
	LogString old_path = par_path + m_base_name;
	LogString new_path = par_path + new_name;
	...
	if (ret) {
		err_log("rename number in %s failed",
			ls2cstring(m_base_name));
	} else {
		m_base_name = new_name;
	}
...
const char* LogFile::get_4digits(const char* s, size_t len)
{
	...
	while (s < endp) {
		if (isdigit(*s)) {
			size_t n = endp - s;
			if (n >= 4 && isdigit(s[1]) && isdigit(s[2]) &&
			    isdigit(s[3])) {
				break;
			}
		}
...
int LogFile::extract_file_time(const char* s, size_t len,
			       FileTime& file_time)
{
	const char* endp = s + len;
	int ret = -1;
