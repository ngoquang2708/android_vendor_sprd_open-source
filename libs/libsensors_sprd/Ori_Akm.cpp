...
OriSensor::OriSensor() :
	SensorBase(NULL, "compass"),
		mPendingMask(0), mInputReader(32)
{
	...
	if (data_fd) {
		if(0==(access("/sys/class/compass/akm8963/", F_OK))) {
			strcpy(input_sysfs_path, "/sys/class/compass/akm8963/");
		}
		else if(0==(access("/sys/class/compass/akm8975/", F_OK))) {
...
int OriSensor::setEnable(int32_t handle, int enabled)
{
	...
	ALOGD("OriSensor: mEnabled[%d] = %d", id, mEnabled[id]);
...
int OriSensor::readEvents(sensors_event_t * data, int count)
{
	...
	while (count && mInputReader.readEvent(&event)) {
		...
		if (type == EV_ABS) {
			...
			for (int j = 0;
			     count && mPendingMask && j < __numSensors; j++) {
				if (mPendingMask & (1 << j)) {
					mPendingMask &= ~(1 << j);
			...
			ALOGE("OriSensor: unknown event (type=%d, code=%d)",
			      type, event->code);
			mInputReader.next();
		}
	}
