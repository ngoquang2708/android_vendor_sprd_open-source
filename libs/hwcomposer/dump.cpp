...
static int dump_layer(const char* path ,const char* pSrc , const char* ptype ,  int width , int height , int format ,int64_t randNum ,  int index , int LayerIndex = 0) {
    ...
    switch(format)
    {
        ...
        case HAL_PIXEL_FORMAT_RGB_888:
            ...
            break;
        case HAL_PIXEL_FORMAT_RGBA_5551:
            sprintf(fileName , "%s%d_%lld_%s_%d_rgba5551_%dx%d_%d.bmp" ,path, cnt,randNum , ptype , LayerIndex , width, height,index);
            break;
        case HAL_PIXEL_FORMAT_RGBA_4444:
            sprintf(fileName , "%s%d_%lld_%s_%d_rgba4444_%dx%d_%d.bmp" ,path,cnt, randNum , ptype , LayerIndex ,width, height,index);
            break;
        case HAL_PIXEL_FORMAT_RGB_565:
