...
int stub_msg_queue_create(unsigned int count, unsigned int *queue_handle)
{
...
int stub_msg_get(unsigned int queue_handle, struct stub_msg *message)
{
...
int stub_msg_post(unsigned int queue_handle, struct stub_msg *message)
{
	struct stub_msg_cxt *msg_cxt = (struct stub_msg_cxt*)queue_handle;
	struct stub_msg     *ori_node = msg_cxt->msg_write;
...
int stub_msg_queue_destroy(unsigned int queue_handle)
{
	struct stub_msg_cxt *msg_cxt = (struct stub_msg_cxt*)queue_handle;
...
static void *stub_common_routine(void *client_data)
{
	...
	while (1) {
		...
		STUB_LOGE("thread 0x%x, data 0x%x, message.msg_type 0x%x, sub-type 0x%x",
			(uint32_t)thread,
			(uint32_t)thread->p_data,
			message.msg_type,
		...
		if (STUB_THREAD_EXIT == ret) {
			STUB_LOGE("[STUB PROC] thread exit 0x%x", (uint32_t)thread);
			break;
		}
	}
...
int stub_thread_create(uint32_t *thread_handle, uint32_t queue_length, msg_process proc_cb, void* p_data)
{
	...
	*thread_handle = 0;
	...
	STUB_LOGE("[STUB PROC] thread 0x%X, data 0x%x", (uint32_t)thread, (uint32_t)p_data);
