...
void SprdCameraHardware::setCameraState(Sprd_camera_state state, state_owner owner)
{
	...
	LOGI("setCameraState: E state: %s, owner: %d", getCameraStateStr(state), owner);
	switch (owner) {
		case STATE_CAMERA:
...
{
	if (mIsPerformanceTestable) {
		sprd_startPerfTracking("receiveJpegPosPicture: E");
	} else {
	...
	print_time();
	if (mIsPerformanceTestable) {
		sprd_stopPerfTracking("%s: receiveJpegPosPicture X: free mCallbackLock!", __func__);
	} else {
		...
		int32_t parm4)
{
	...
	camera_set_capture_trace(0);
	LOGI("HandleEncode X, state = %s", getCameraStateStr(getCaptureState()));
	if (mIsPerformanceTestable) {
		...
		int32_t parm4)
{
	...
	LOGI("HandleFocus X, state = %s", getCameraStateStr(getFocusState()));
	if (mIsPerformanceTestable) {
