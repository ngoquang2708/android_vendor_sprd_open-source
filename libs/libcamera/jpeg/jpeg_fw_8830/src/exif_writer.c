                                    ...
                                    uint32 *end_offset_ptr)
{
	...
	JPEG_PRINT_LOW("Jpeg_Write_APP3_ExifIFD: E!!!\n");
	...
	entries = 2;
	Jpeg_SetWritePos(context_ptr, begin_offset);
	JPEG_PRINT_LOW("begin_offset = 0x%x", begin_offset);
	JPEG_WRITE_DATA(Jpeg_WriteW, context_ptr, entries, return JPEG_FAILED);
	entries_offset = begin_offset;

	ifd_offset = Jpeg_GetWritePos(context_ptr);
	JPEG_PRINT_LOW("ifd_offset = 0x%x", ifd_offset);
	ifd_value_offset = ifd_offset + entries * IFD_HEAD_LENGTH + 4;
	ifd_value_offset = JPEG_ALIGN_4(ifd_value_offset);
	JPEG_PRINT_LOW("ifd_value_offset = 0x%x", ifd_value_offset);
                                 ...
                                 uint32 *app3_size_ptr)
{
	...
	JPEG_PRINT_LOW("[Jpeg_WriteAPP3] :E!!!\n");
                                      ...
                                      JINF_WEXIF_OUT_PARAM_T *out_param_ptr)
{
	...
	if (NULL != in_param_ptr->exif_isp_info && NULL != app3_buf_ptr && app3_buf_size > 0) {
	    ret = Jpeg_WriteAPP3(app3_buf_ptr,
	                         ...
	                         &app3_size);
	    JPEG_PRINT_LOW("[JPEG_AddExifToFile] :after Jpeg_WriteAPP3, ret = %d,  app3_size = %d\n", ret, app3_size);
		if (JPEG_SUCCESS == ret) {
		    free_buf_size = (uint32)in_param_ptr->src_jpeg_buf_ptr
	...
	if(app3_size > 0) {
	    target_buf_ptr = in_param_ptr->src_jpeg_buf_ptr - app3_size;

    out_param_ptr->output_buf_ptr = target_buf_ptr;
		out_param_ptr->output_size = app3_size + in_param_ptr->src_jpeg_size;

	    #ifdef EXIF_DEBUG
	    JPEG_PRINT_LOW("exif_debug: target_buf_ptr:0x%x , app3_size: 0x%x\n", target_buf_ptr, app3_size);
	    #endif
	    memcpy(target_buf_ptr, app3_buf_ptr, app3_size);
	}
    target_buf_ptr = in_param_ptr->src_jpeg_buf_ptr - app1_size - app3_size - JPEG_HEADER_BYTE;
    out_param_ptr->output_buf_ptr = target_buf_ptr;
    out_param_ptr->output_size = app1_size + app3_size + in_param_ptr->src_jpeg_size + JPEG_HEADER_BYTE;
    //write SOI marker
    *target_buf_ptr++ = M_MARKER;
    *target_buf_ptr++ = M_SOI;
#ifdef EXIF_DEBUG
    JPEG_PRINT_LOW("target buf:0x%x,app1_size %d.",
		(uint32_t)target_buf_ptr,app1_size);
#endif
...
/*   for(i=0;i<app1_size;i++) {
    ...
    #ifdef EXIF_DEBUG
    JPEG_PRINT_LOW("exif_debug: target_buf_ptr:0x%x , in_param_ptr->src_jpeg_buf_ptr: 0x%x, in_param_ptr->target_buf_ptr: 0x%x\n",
    target_buf_ptr, in_param_ptr->src_jpeg_buf_ptr,  in_param_ptr->target_buf_ptr);
    #endif
