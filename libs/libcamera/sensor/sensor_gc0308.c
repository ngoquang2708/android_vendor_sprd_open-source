...
static uint32_t set_GC0308_anti_flicker(uint32_t param )
{
	switch (param) {
	case FLICKER_50HZ:
		GC0308_WriteReg(0x01, 0x0A);
		GC0308_WriteReg(0x02, 0x06);
		GC0308_WriteReg(0x0f, 0x11);
		GC0308_WriteReg(0xe2, 0x00);	//anti-flicker step [11:8]
		GC0308_WriteReg(0xe3, 0x7D);   //anti-flicker step [7:0]
		GC0308_WriteReg(0xe4, 0x02);   //exp level 1  16.67fps
		GC0308_WriteReg(0xe5, 0xEE);
		GC0308_WriteReg(0xe6, 0x03);   //exp level 2  12.5fps
		GC0308_WriteReg(0xe7, 0xE8);
		GC0308_WriteReg(0xe8, 0x04);   //exp level 3  10fps
		GC0308_WriteReg(0xe9, 0xE2);
		GC0308_WriteReg(0xea, 0x07);   //exp level 4  6.25fps
		GC0308_WriteReg(0xeb, 0xD0);
	break;
	case FLICKER_60HZ:
		GC0308_WriteReg(0x01, 0x32);
		GC0308_WriteReg(0x02, 0x38);
		GC0308_WriteReg(0x0f, 0x11);
		GC0308_WriteReg(0xe2, 0x00);	//anti-flicker step [11:8]
		GC0308_WriteReg(0xe3, 0x64);	//anti-flicker step [7:0]

		GC0308_WriteReg(0xe4, 0x03);	//exp level 1  15.00fps
		GC0308_WriteReg(0xe5, 0x20);
		GC0308_WriteReg(0xe6, 0x03);	//exp level 2  12fps
		GC0308_WriteReg(0xe7, 0xE8);
		GC0308_WriteReg(0xe8, 0x04);	//exp level 3  10fps
		GC0308_WriteReg(0xe9, 0xB0);
		GC0308_WriteReg(0xea, 0x06);	//exp level 4  7.5fps
		GC0308_WriteReg(0xeb, 0x40);
	break;
	default:
	break;
	}
