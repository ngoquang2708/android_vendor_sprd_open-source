...
{0x29,0x20},  //lsc_dec_fac     进dummy态退shading 功能有问题，需关掉                  
...
{0xdc,0x03}, //skin_sharp_sel肤色降锐化                       
{0x05,0x00}, //skin_num_th2排除肤色降锐化对分辨率卡引起的干扰 
...
{0x7b,0x40},
 //auto_sat                        
{0xfd,0x01},
{0xc2,0xff},  //u_v_th_outdoor白色物体表面有彩色噪声降低此值    
{0xc3,0xff},  //u_v_th_nr                                       
...
{0x03,0x30}, //ctf_range_thr   可以排除灰板场景的阈值   
...
/* const SENSOR_REG_T SP2529_video_mode_nor_tab_60hz[][35]=
{
//video  preview
	{
				//48M 60Hz fix 9fps uxga
			{0xfd,0x00},
			{0x03,0x02},
			{0x04,0x2e},
			{0x05,0x00},
...
const SENSOR_REG_T SP2529_video_mode_nor_tab_50hz[][35]=
{
//video  preview
	{
				//48M 50Hz fix 9fps uxga
			{0xfd,0x00},
			{0x03,0x02},
			{0x04,0xa0},
			{0x05,0x00},
...
static uint32_t set_SP2529_video_mode(uint32_t mode)
{
	...
	{
				/*video normal mode 2Pll fix 9fps 50hz*/ 
				SP2529_WriteReg(0xfd,0x00);
				SP2529_WriteReg(0x03,0x02);
				SP2529_WriteReg(0x04,0xa0);
				SP2529_WriteReg(0x05,0x00);
	...
	{
			/*video normal mode 2Pll fix 9fps 60hz*/                           
				SP2529_WriteReg(0xfd,0x00);       
				SP2529_WriteReg(0x03,0x02);       
				SP2529_WriteReg(0x04,0x2e);       
				SP2529_WriteReg(0x05,0x00);       
		...
		CMR_LOGV("SP2529 SENSOR: set_video_mode2222222");
	}
    return 0;
...
/*// return 0;
    ...
    SENSOR_REG_T* sensor_reg_ptr = PNULL;
    uint8 tempregval = 0;
    SENSOR_PRINT(" xg:set_SP2529_video_mode ,%d,%d\r\n",mode,Antiflicker);
 //   SCI_ASSERT(mode <=DCAMERA_MODE_MAX);
	...
	if(Antiflicker ==DCAMERA_FLICKER_50HZ )
	{
	 ...
	 //           sensor_reg_ptr = (SENSOR_REG_T*)SP2529_video_mode_nor_tab_50hz[mode];
	...
	//            sensor_reg_ptr = (SENSOR_REG_T*)SP2529_video_mode_nor_tab_60hz[mode];
...
static uint32_t set_preview_mode(uint32_t preview_mode)
{
	...
	sensor_reg_ptr = (SENSOR_REG_T*)SP2529_YUV_640x480;
	...
	switch (preview_mode)
	{
		...
		if(Antiflicker== DCAMERA_FLICKER_50HZ)
		{
		//SCI_TRACE_LOW("normal mode 50hz\r\n");						
		...
		{
		//SCI_TRACE_LOW("normal mode 60hz\r\n");					
		//24M 60Hz 2pll uxga fix 9ps
		SP2529_WriteReg(0xfd,0x00);
		SP2529_WriteReg(0x03,0x02);
		SP2529_WriteReg(0x04,0x2e);
		...
		//48M 50Hz fix 7fps uxga
		 SP2529_WriteReg(0xfd,0x00);
		SP2529_WriteReg(0x03,0x02);
		SP2529_WriteReg(0x04,0x0a);
		SP2529_WriteReg(0x05,0x00);
		SP2529_WriteReg(0x06,0x00);
		SP2529_WriteReg(0x07,0x00);
		SP2529_WriteReg(0x08,0x00);
		SP2529_WriteReg(0x09,0x01);
		SP2529_WriteReg(0x0a,0xc9);
		SP2529_WriteReg(0xfd,0x01);
		SP2529_WriteReg(0xf0,0x00);
		SP2529_WriteReg(0xf7,0x57);
		SP2529_WriteReg(0xf8,0x49);
		SP2529_WriteReg(0x02,0x0e);
		SP2529_WriteReg(0x03,0x01);
		SP2529_WriteReg(0x06,0x57);
		SP2529_WriteReg(0x07,0x00);
		SP2529_WriteReg(0x08,0x01);
		SP2529_WriteReg(0x09,0x00);
		SP2529_WriteReg(0xfd,0x02);
		SP2529_WriteReg(0x3d,0x11);
		SP2529_WriteReg(0x3e,0x49);
		SP2529_WriteReg(0x3f,0x00);
		SP2529_WriteReg(0x88,0xe2);
		SP2529_WriteReg(0x89,0x03);
		SP2529_WriteReg(0x8a,0x75);
		SP2529_WriteReg(0xfd,0x02);
		SP2529_WriteReg(0xbe,0xc2);
		SP2529_WriteReg(0xbf,0x04);
		SP2529_WriteReg(0xd0,0xc2);
		SP2529_WriteReg(0xd1,0x04);
		SP2529_WriteReg(0xc9,0xc2);
		SP2529_WriteReg(0xca,0x04);   
		}
		else
		{
		//SCI_TRACE_LOW("night mode 60hz\r\n");
		//48M 60Hz fix 7fps uxga
		  SP2529_WriteReg(0xfd,0x00);
		SP2529_WriteReg(0x03,0x01);
		SP2529_WriteReg(0x04,0xb0);
		SP2529_WriteReg(0x05,0x00);
		SP2529_WriteReg(0x06,0x00);
		SP2529_WriteReg(0x07,0x00);
		SP2529_WriteReg(0x08,0x00);
		SP2529_WriteReg(0x09,0x01);
		SP2529_WriteReg(0x0a,0xd3);
		SP2529_WriteReg(0xfd,0x01);
		SP2529_WriteReg(0xf0,0x00);
		SP2529_WriteReg(0xf7,0x48);
		SP2529_WriteReg(0xf8,0x48);
		SP2529_WriteReg(0x02,0x11);
		SP2529_WriteReg(0x03,0x01);
		SP2529_WriteReg(0x06,0x48);
		SP2529_WriteReg(0x07,0x00);
		SP2529_WriteReg(0x08,0x01);
		SP2529_WriteReg(0x09,0x00);
		SP2529_WriteReg(0xfd,0x02);
		SP2529_WriteReg(0x3d,0x11);
		SP2529_WriteReg(0x3e,0x48);
		SP2529_WriteReg(0x3f,0x00);
		SP2529_WriteReg(0x88,0x1c);
		SP2529_WriteReg(0x89,0x1c);
		SP2529_WriteReg(0x8a,0x77);
		SP2529_WriteReg(0xfd,0x02);
