...
char fixnv_ori_path[NV_PATH_MAX_LEN];
char fixnv_bak_path[NV_PATH_MAX_LEN];
char runnv_ori_path[NV_PATH_MAX_LEN];
char runnv_bak_path[NV_PATH_MAX_LEN];
uint32 fixnv_size, runnv_size;
extern char argv1[10];
extern char channel_path[PROPERTY_VALUE_MAX];

int initArgs(void)
{
	char fixnv[PROPERTY_VALUE_MAX], runnv[PROPERTY_VALUE_MAX];
	char fixnv_property[50], runnv_property[50];
	char ro_prop[20];
	char persist_prop[32];
	char partition_path[PROPERTY_VALUE_MAX];
	char nvp[50];
	char path_char[PROPERTY_VALUE_MAX];
	...
	if (NV_PATH_MAX_LEN > strlen(partition_path) + strlen(path_char) + strlen("fixnv1")) {
	...
	if (NV_PATH_MAX_LEN > strlen(partition_path) + strlen(path_char) + strlen("runtimenv1")) {
...
{
	if (strlen(fixnv_ori_path) > NV_PATH_MAX_LEN || strlen(fixnv_bak_path) > NV_PATH_MAX_LEN
	    || strlen(runnv_ori_path) > NV_PATH_MAX_LEN || strlen(runnv_bak_path) > NV_PATH_MAX_LEN) {
	...
	NVITEM_PRINT("i = 0\t0x%x\t%32s\t%32s\t0x%8x\n", _ramdiskCfg[0].partId, _ramdiskCfg[0].image_path, _ramdiskCfg[0].imageBak_path,
	...
	NVITEM_PRINT("i = 1 \t0x%x\t%32s\t%32s\t0x%8x\n", _ramdiskCfg[1].partId, _ramdiskCfg[1].image_path, _ramdiskCfg[1].imageBak_path,
